...
2. **Confidentiality across the company boundary.** Jake's live sessions are named client work: enviz-security, messina-dashboard, hero-hq-lms-platform. Whatever seshi sends lands permanently in Dan's transcripts, under Dan's account, governed by Dan's training opt-in. *Mitigation: show what is leaving, per message, before it leaves. State in the README that the default scope is personal and open-source work. Make context sharing an explicit per-conversation flag.*
