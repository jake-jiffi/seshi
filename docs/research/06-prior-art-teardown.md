...
- Do not copy the pairing artefact. `retalk id --invite-message` (cli.py:700-732) renders a copy-paste BASH BLOCK that tells the peer to `pip install -U retalk`, run `retalk init --relay ... --passphrase "<PRIVATE-PASSPHRASE>"`, then `retalk request <32-hex-fingerprint> --code <22-char-code> --peer <name>`. The comments even work around stock macOS zsh having interactive comments off. That is not a short code pasted into a connector.
...
Each agent type is declared by a flat key=value manifest that is read as DATA and never sourced (`/Users/you/dev/jiffi-seshi/reference/agmsg/scripts/drivers/types/claude-code/type.conf`):
